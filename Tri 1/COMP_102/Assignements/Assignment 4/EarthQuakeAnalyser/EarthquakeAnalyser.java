...
public class EarthquakeAnalyser extends Earthquake
{
    ...
    public void findPairs()
    {
        ...
        while (i < earthquakes.size()) {
            ...
            while (j < this.earthquakes.size()) {
                if (this.quake.distanceTo(earthquakes.get(j)) <= 1 &&
                    this.quake.timeBetween(earthquakes.get(j)) >= 1440 &&
                    Math.abs(this.quake.getDepth() - earthquakes.get(j).getDepth()) <= 1){
                    UI.printf("%s and %s: distance=%1.2f, depth diff=%1.2f, days apart=%3.0f\n",
                               this.quake.getID(), earthquakes.get(j).getID(), this.quake.distanceTo(earthquakes.get(j)),
                               Math.abs(this.quake.getDepth() - earthquakes.get(j).getDepth()),
                               (this.quake.timeBetween(earthquakes.get(j))) / 1440);
                }
                j = j + 1;
            }
    ...
    /**
     * Constructs a new ArrayList containing every earthquake with a magnitude that is at least 6 For each earthquake on this list
     * finds a list of all the "follow-on" earthquakes: later earthquakes within a distance of 10km and depth difference <= 10km.
     * If there are at least two follow-on earthquakes, then it prints out the full details of the big earthquake followed by ID,
     * magnitude and days since the big one for each follow-on earthquake
     */
