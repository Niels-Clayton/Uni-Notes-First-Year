...
public class Patient implements Comparable<Patient>{
    ...
    public int compareTo(Patient other){
        if(this.priority < other.getPriority()){return -1;} // The patient with highest priority is always first
        
        else if(priority == other.getPriority()){  // If they have the same priority, the one waiting longest is first
            if(arrival < other.getArrivalTime()){return -1;}
            else if (arrival > other.getArrivalTime()){return 1;}
            else {return 0;}
        }
        
        else {return 1;}  // If lower priority, put after
    }

    /** toString */
    public String toString(){
        return name+", priority:"+priority+", Arrival time:"+arrival+", " +
        "treatment time: "+ totalTreatmentTime+ " wait: "+ totalWaitTime+ "\n     "+ treatments+"/"+treatmentTimes;
    }
    ...
    public void makeRandomTreatments(){
        ...
        if (nextNum < 0.02){ //make sure that if people go to the crematory, its last as they died
            treatments.add("Crematory");
