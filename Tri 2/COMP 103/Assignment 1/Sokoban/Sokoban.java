...
public class Sokoban {
    ...
    private Stack<ActionRecord> undo = new Stack<ActionRecord>(); //the stack that will hold the undo action records
    private Stack<ActionRecord> redo = new Stack<ActionRecord>(); //the stack that will hold the redo action records
    ...
    public void doKey(String key) {
        ...
        else if (key.equals("z") || key.equals("u")) undo();
        else if (key.equals("r")) redo();
    }

    public void undo(){
        if(undo.isEmpty()) return;
        ActionRecord undoMove = undo.pop();  //pop an action record from the undo stack
        if(undoMove.isMove()){               //if the ActionRecord is a move, move worker in the opposite direction
            String backDirection = opposite(undoMove.direction());
            workerDir = undoMove.direction();
            move(backDirection);
        }
        else{
            String direction = undoMove.direction();
            workerDir = undoMove.direction();
            pull(direction);                //run the pull method
        }
        redo.push(undoMove);                //push the undo action record onto the redo stack.
    }

    public void redo(){
        if(redo.isEmpty()) return;
        ActionRecord redoMove = redo.pop();     //pop an action record from the redo stack
        if(redoMove.isMove()){                  //if the ActionRecord is a move, move worker in the direction of the ActionRecord
            String backDirection = redoMove.direction();
            workerDir = backDirection;
            move(backDirection);
        }
        else{
            String direction = redoMove.direction(); //if the action is a move, push in the direction of the record
            workerDir = redoMove.direction();
            push(direction);
        }
        undo.push(redoMove);
    }
    ...
    public void moveOrPush(String direction) {
        ...
        if ( cells[nextP.row][nextP.col].hasBox() &&
            ...
            push(direction);
            undo.push(new ActionRecord("push", direction));  //push a new action record to the undo stack
        ...
        // is the next cell free for the worker to move into?
            ...
            move(direction);
            undo.push(new ActionRecord("move", direction));  //push a new action record to the undo stack
        }
    }
    ...
    public void pull(String direction) {
        String pullDirection = opposite(direction);

        Position boxPos = workerPos.next(direction);                  // where box is
        Position oldWorkerPosition = workerPos.next(pullDirection);   //where the worker was
        Position oldBoxPos = workerPos;                               //where the box was

        cells[boxPos.row][boxPos.col].removeBox();                    // remove box from current cell
        cells[oldBoxPos.row][oldBoxPos.col].addBox();                 //put box back where it was

        drawCell(boxPos);                                             //redisplay cell where box was
        drawCell(oldBoxPos);                                          //redisplay cell with the box back in it

        workerPos = oldWorkerPosition;
        drawWorker();
